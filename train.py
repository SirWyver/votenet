...
parser.add_argument("--overfit", action="store_true", help="overfit?")
parser.add_argument("--val_quick", action="store_true", help="overfit?")
...
if FLAGS.dataset == "sunrgbd":
    ...
    TRAIN_DATASET = FSNDetectionDataset(
        "train" if not FLAGS.overfit else "overfit",
        num_points=NUM_POINT,
        augment=True,
        use_color=FLAGS.use_color,
        use_height=(not FLAGS.no_height),
...
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=BATCH_SIZE, shuffle=True, num_workers=FLAGS.num_workers, worker_init_fn=my_worker_init_fn)
TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=BATCH_SIZE, shuffle=False, num_workers=FLAGS.num_workers, worker_init_fn=my_worker_init_fn)
...
def evaluate_one_epoch():
    ...
    TEST_VISUALIZER.log_scalars(
        {key: stat_dict[key] / float(batch_idx + 1) for key in stat_dict}, (EPOCH_CNT + 1) * len(TRAIN_DATALOADER) * BATCH_SIZE
    )
    for key in sorted(stat_dict.keys()):
